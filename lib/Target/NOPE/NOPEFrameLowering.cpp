...
void NOPEFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (hasFP(MF)) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(NOPE::PUSH16r))
      .addReg(NOPE::FP, RegState::Kill);

    // Update FP with the new base value...
    BuildMI(MBB, MBBI, DL, TII.get(NOPE::MOV16rr), NOPE::FP)
      .addReg(NOPE::SP);
  ...
  if (NumBytes) { // adjust stack pointer: SP -= numbytes
    ...
    if (NumBytes) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL, TII.get(NOPE::SUB16ri), NOPE::SP)
...
void NOPEFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  switch (RetOpcode) {
  case NOPE::RET:
  case NOPE::RETI: break;  // These are ok
  default:
  ...
  if (hasFP(MF)) {
    ...
    // pop FP.
    BuildMI(MBB, MBBI, DL, TII.get(NOPE::POP16r), NOPE::FP);
  } else
